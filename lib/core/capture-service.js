...
const CAPTURE_NOISE_PATTERNS = Object.freeze([
  /✅\s*New session started\b/i,
  /#\s*Recovery Quickstart\b/i,
  /"type"\s*:\s*"thinking"/i,
  /"type"\s*:\s*"reasoning"/i,
  /"thinkingSignature"\s*:/i,
  /"encrypted_content"\s*:/i,
  /"id"\s*:\s*"rs_[^"]+"/i,
  /Determining appropriate skill for heartbeat/i,
]);

const isCaptureNoise = (sourceText = '') => {
  const text = String(sourceText || '');
  if (!text) return false;
  return CAPTURE_NOISE_PATTERNS.some((pattern) => pattern.test(text));
};

const MEMORY_NOTE_HINT_RE = /<memory_note\b[^>]*\b(?:type|confidence|tags|concept|subject|scope)\s*=/i;

const shouldQueueNoNotes = (event = {}, sourceText = '') => {
  const text = String(sourceText || '');

  // OpenAI reasoning/"thinkingSignature" artifacts can appear in after_turn/agent_end payloads.
  // If we see those markers, do not create parser-failure review rows.
  if (isCaptureNoise(text)) return '';

  if (MEMORY_NOTE_HINT_RE.test(text)) return 'capture_note_parse_failed';
